...
def generate_stats(directory_file, contents_file, output_file, directory_enabled=True, codebase_enabled=True):
    ...
    if directory_enabled and os.path.exists(directory_file):
    ...
    if codebase_enabled and os.path.exists(contents_file):
    ...
    with open(output_file, 'w', encoding='utf-8') as stats_file:
        ...
        if directory_enabled and file_types:
        ...
        if directory_enabled and excluded_files:
            stats_file.write("\nExcluded Files\n")
            stats_file.write("=============\n")
...
if __name__ == "__main__":
    ...
    if STATS_ENABLED:
        generate_stats(directory_file, codebase_file, stats_file, DIRECTORY_ENABLED, CODEBASE_ENABLED)

    print(f"\nProcess completed!")
    if DIRECTORY_ENABLED:
        print(f"Directory mapping: {directory_file}")
    if CODEBASE_ENABLED:
        print(f"Codebase content: {codebase_file}")
    if STATS_ENABLED:
